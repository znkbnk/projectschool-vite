...
function MusicAcademy() {
  ...
  useEffect(() => {
    const refreshCompletedTasks = () => {
      try {
        const saved = localStorage.getItem("Workshop_completedTasks");
        const storedTasks = saved ? JSON.parse(saved) : {};
  ...
  const handleDownloadCode = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/code/workshop-full-code`,
        {
          method: "GET",
      ...
      a.href = url;
      a.download = "workshop-full-code.zip";
    ...
    <>
      ...
      <div className='header'>
        <h1 className='component-title'>
          Learn React: Build a Music Academy App
        </h1>
        <p className='header-subtitle'>
          Full MERN stack workshop — booking system, payments, admin dashboard,
        ...
        {!isSubscribed && (
          <div className='access-info-banner'>
            ...
            <span>
              ...
              to unlock all {Workshop.length} lessons with backend, auth,
              payments & deployment
            </span>
      ...
      <ProgressBar tasks={Workshop} completedTasks={completedTasks} />
      <FilterSortButtons
