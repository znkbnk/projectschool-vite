...
import { dataFetchingData} from "../data/dataFetchingData";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import "../styles/usestateGuide.css";
import SyntaxHighlighter from "react-syntax-highlighter";
import PropTypes from "prop-types";
// import UseEffectLearningJourneySection from "./UseEffectLearningJourneySection";
...
// Don't link useEffect on its own guide page
const EXCLUDE = ["dataFetching"];
...
const DataFetchingGuide = () => {
  const guide = dataFetchingData[0]; // Use the single guide
  // Initialize activeTab with first tab for each examples section
  ...
  useEffect(() => {
    const saved = localStorage.getItem("dataFetchingCheckedTitles");
    if (saved) {
      setCheckedTitles(JSON.parse(saved));
    }
  }, []);
  ...
  // Function to scroll to Table of Contents
  const scrollToTop = () => {
  ...
  // Save checkedTitles to localStorage with unique key for useEffect
  useEffect(() => {
    localStorage.setItem(
      "dataFetchingCheckedTitles",
  ...
  const handleTabKeyDown = (event, sectionId, tabs, index) => {
    if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
      ...
      if (nextTab) {
        nextTab.focus(); // Ensure focus moves to the new tab
      }
    }
  };
          ...
          <article>
            <section className='hero-section'>
              <h1 className='title'>
                ...
                <span className='title-sub'>A Comprehensive Guide</span>
              </h1>
                ...
                >
                  {paragraph.type === "text" && (
                    <p className='article-paragraph'>
                      {linkifyKeywords(paragraph.content, EXCLUDE)}
                    </p>
                  )}
                  ...
                  {paragraph.type === "practiceTask" && (
                    <div className='practice-task'>
                      ...
                      <p className='article-paragraph'>
                        {linkifyKeywords(paragraph.hint, EXCLUDE)}
                      </p>
            ...
            {/* Features Grid */}
            {/* <UseEffectLearningJourneySection /> */}
            {showBackToTop && (
              <button
...
export default DataFetchingGuide;
