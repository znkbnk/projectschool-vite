...
import { reactPerformanceData } from "../data/reactPerformanceData";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import "../styles/usestateGuide.css";
import SyntaxHighlighter from "react-syntax-highlighter";
import PropTypes from "prop-types";
// import UseStateLearningJourneySection from "./UseStateLearningJourneySection";
...
// Don't link 'guide' on its own guide page
const EXCLUDE = [""];
...
const detectLanguage = (code) => {
  if (code.includes("import React") || code.includes("useState") || (code.includes("<") && code.includes("/>"))) {
    return "jsx";
  }
  if (code.includes("const ") || code.includes("let ") || code.includes("function") || code.includes("=>")) {
    return "javascript";
  }
...
const ReactPerformanceGuide = () => {
  const guide = reactPerformanceData[0];
 ...
 const [checkedTitles, setCheckedTitles] = useState(() => {
  ...
  try {
    const saved = localStorage.getItem("reactPerformanceCheckedTitles");
    return saved ? JSON.parse(saved) : {};
  } catch (error) {
    console.error("Error parsing checked titles:", error);
    return {};
  }
});
  ...
  useEffect(() => {
    localStorage.setItem("reactPerformanceCheckedTitles", JSON.stringify(checkedTitles));
  }, [checkedTitles]);
...
const toc = useMemo(() => {
  if (!guide?.paragraphs) return [];
  
  return guide.paragraphs
      ...
      <main className='main'>
        <div className='guide'>
          <article>
            ...
            {/* Features Grid */}
            {/* <UseStateLearningJourneySection /> */}
            {showBackToTop && (
              <button
...
export default ReactPerformanceGuide;