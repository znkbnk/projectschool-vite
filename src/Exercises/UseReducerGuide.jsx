...
import { useReducerData } from "../data/useReducerData";
...
// Don't link useState on its own guide page
const EXCLUDE = ["useReducer"];
...
const UseReducerGuide = () => {
  const guide = useReducerData[0];
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    guide?.paragraphs
      ...
      ?.forEach((ex, index) => {
        if (ex.tabs?.length > 0) {
          initial[`example-${index}`] = ex.tabs[0].id;
        }
      });
    return initial;
  });
  ...
  const [checkedTitles, setCheckedTitles] = useState(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("useReducerCheckedTitles");
...
export default UseReducerGuide;
