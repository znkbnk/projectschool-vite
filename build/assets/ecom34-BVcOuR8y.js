...
const AdminDashboard = () => {
      ...
      try {
        ...
        const [statsRes, productsRes, ordersRes, usersRes, categoriesRes] =
          await Promise.all([
            ...
            fetchWithAuth(\`\${API_URL}/api/products?limit=100\`),
            fetchWithAuth(\`\${API_URL}/api/orders/all\`),
            fetchWithAuth(\`\${API_URL}/api/users\`),
            fetchWithAuth(\`\${API_URL}/api/products/categories\`),
          ]);
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        res = await fetchWithAuth(\`\${API_URL}/api/products\`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(productData),
        });
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      ...
      const res = await fetchWithAuth(\`\${API_URL}/api/products/\${productId}\`, {
        method: 'DELETE',
      });
  ...
  const handleToggleAdmin = async (userId, currentRole) => {
    try {
      const token = localStorage.getItem('accessToken');
      await fetchWithAuth(\`\${API_URL}/api/users/\${userId}/role\`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: \`Bearer \${token}\`,
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.orderTotalSection}>
                ...
                <div
                  className={\`\${styles.orderTotalRow} \${styles.orderGrandTotal}\`}
                >
                  <span>Total</span>
...
`];export{e as default};
//# sourceMappingURL=ecom34-BVcOuR8y.js.map